...
function wp_password_change_notification(&$user) {
	...
	if ( 0 !== strcasecmp( $user->user_email, get_option( 'admin_email' ) ) ) {
		$message = sprintf(__('Password Lost and Changed for user: %s'), $user->user_email) . "\r\n";
		$message .= sprintf(__('<br> CNPJ: %s'), $user->cnpj) . "\r\n";
		$message .= sprintf(__('<br> Razão Social: %s'), $user->razao_social) . "\r\n";
		// The blogname option is escaped with esc_html on the way into the database in sanitize_option
		// we want to reverse this for the plain text arena of emails.
		$blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);
...
function wp_new_user_notification($user_id, $plaintext_pass = '') {
	...
	$message  = sprintf(__('Um novo usuário se registrou no site %s:'), $blogname) . "\r\n\r\n";
	$message .= sprintf(__('Nome: %s'), $user->first_name) . "\r\n";
	$message .= sprintf(__('Usuario: %s'), $user->user_login) . "\r\n";
	$message .= sprintf(__('E-mail: %s'), $user->user_email) . "\r\n";
	$message .= sprintf(__('Razão Social: %s'), $user->razao_social) . "\r\n";
	$message .= sprintf(__('Nome Fantasia: %s'), $user->nome_fantasia) . "\r\n";	
	$message .= sprintf(__('CNPJ: %s'), $user->cnpj) . "\r\n";
	$message .= sprintf(__('Tel: %s'), $user->tel) . "\r\n";
	$message .= sprintf(__('Cel: %s'), $user->cel) . "\r\n";
	$message .= sprintf(__('CEP: %s'), $user->cep) . "\r\n";
	$message .= sprintf(__('Endereço: %s'), $user->endereco) . "\r\n";
	$message .= sprintf(__('Cidade: %s'), $user->cidade) . "\r\n";
	$message .= sprintf(__('Estado: %s'), $user->estado) . "\r\n";
	$message .= sprintf('Clique no link abaixo para fazer a análise do usuário e aprová-lo:') . "\r\n\r\n";
	$message .= sprintf('http://www.homecollection.com.br/wp-admin/users.php?page=new-user-approve-admin') . "\r\n";

	@wp_mail(get_option('admin_email'), sprintf(__('[%s] Novo registro de usuário'), $blogname), $message);

	if ( empty($plaintext_pass) )
		return;
$message  = sprintf(__('E-mail: %s'), $user->user_email) . "\r\n";
	$message  = sprintf(__('Usuário: %s'), $user->user_login) . "\r\n";
	$message .= sprintf(__('Senha: %s'), $plaintext_pass) . "\r\n";
	$message = sprintf('Seu cadastro está sujeito a aprovação e em breve entraremos em contato.') . "\r\n";

	wp_mail($user->user_email, sprintf(__('[%s] Seu cadastro em nosso site.'), $blogname), $message);
